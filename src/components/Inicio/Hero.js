...
const Hero = () => {
    ...
    if (!content) {
        return <div className="skeleton rounded-none w-full h-screen"></div>
    }
    ...
    return (
        <div className="h-[90vh] relative w-full overflow-hidden">
            ...
            <div className="flex flex-col absolute glass w-1/4 ml-32 mt-40 justify-center p-12 rounded-sm text-slate-300">
                <h2 className="text-3xl font-bold mb-2">{content.title}</h2>
                <p className='text-md text-slate-300 font-light'>{content.description}</p>
                <button className=" bg-yellow-600/50 font-normal p-4 px-5 mt-7 text-md w-fit text-slate-100 rounded-sm">
                Ver ahora
            </button>
            </div>
